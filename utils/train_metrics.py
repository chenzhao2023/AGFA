...
def metrics3d(pred, label, batch_size):     # pred.shape : [16, 2, 96, 96, 96]
    # pred = torch.argmax(pred, dim=1)  # for CE loss series   # pred.shape : [16, 96, 96, 96]
    outputs = (pred.data.cpu().numpy() * 255).astype(np.uint8)
    labels = (label.data.cpu().numpy() * 255).astype(np.uint8)
    # outputs = outputs.squeeze(1)  # for MSELoss()
    # labels = labels.squeeze(1)  # for MSELoss()
    ...
    Recall, Precision, fp, IoU, Dice = 0, 0, 0, 0,0
    for i in range(batch_size):
        img = outputs[i, :, :, :]
        gt = labels[i, :, :, :]
        # tpr, fnr, fpr, iou= metrics_3d(img, gt)
        recall, precision, fpr, dice = metrics_3d(img, gt)
        # dcr = Dice(img, gt)
        Recall += recall
        Precision += precision
        fp += fpr
        # IoU += iou
        Dice += dice
    return Recall, Precision, fp, Dice
