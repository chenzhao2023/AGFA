...
class AGFANet(nn.Module):
    ...
    def forward(self, x):
        enc_input = self.enc_input(x)       # [1, 16, 128, 160, 160])
        down1 = self.downsample(enc_input)  # [1, 16, 64, 80, 80])

        enc1 = self.encoder1(down1)         # [1, 32, 64, 80, 80])
        down2 = self.downsample(enc1)       # [1, 32, 32, 40, 40])
        down2= self.combAttention1(down2)   # 

        enc2 = self.encoder2(down2)         # [1, 64, 32, 40, 40])
        down3 = self.downsample(enc2)       # [1, 64, 16, 20, 20])
        down3= self.combAttention2(down3)   # 
        
        enc3 = self.encoder3(down3)         #  enc3:  [1, 128, 16, 20, 20])
        down4 = self.downsample(enc3)       # [1, 128, 8, 10, 10])
        down4= self.combAttention3(down4)   # 
 
        input_feature = self.encoder4(down4)  #  input_feature:  [1, 256, 8, 10, 10])
        
        input_feature=self.split_channelFeature_comb(input_feature)
        
        # Do Attenttion operations here
        attention = self.affinity_attention(input_feature)    #  attention:  [1, 256, 8, 10, 10])
        attention_fuse = torch.add(input_feature, attention)  #  attention_fuse:  [1, 256, 8, 10, 10])
        #attention_fuse = input_feature + attention

        up4 = self.deconv4(attention_fuse)      #  up4:  [1, 128, 16, 20, 20])
        up4 = torch.cat((enc3, up4), dim=1)     #  up4:  [1, 256, 16, 20, 20])
        dec4 = self.decoder4(up4)       #  dec4:  [1, 128, 16, 20, 20])
        dec4= self.combAttention3(dec4)   
        
        up3 = self.deconv3(dec4)            # [1, 64, 32, 40, 40])
        # print("up3 : ", up3)
        up3 = torch.cat((enc2, up3), dim=1) # [1, 128, 32, 40, 40])
        dec3 = self.decoder3(up3)           # [1, 64, 32, 40, 40])
        dec3= self.combAttention2(dec3)  

        up2 = self.deconv2(dec3)        # [1, 32, 64, 80, 80])
        up2 = torch.cat((enc1, up2), dim=1) # [1, 64, 64, 80, 80])
        dec2 = self.decoder2(up2)       # [1, 32, 64, 80, 80])
        dec2= self.combAttention1(dec2) 

        up1 = self.deconv1(dec2)        # [1, 16, 128, 160, 160])
        up1 = torch.cat((enc_input, up1), dim=1)  # [1, 32, 128, 160, 160])
        dec1 = self.decoder1(up1)       # [1, 16, 128, 160, 160])
        
        # HFIM
        lay3=self.sigmoid(torch.cat((self.deconv3(dec4),dec3),dim=1)) # [1, 128, 32, 40, 40])
        lay3 = self.decoder3(lay3) 
        lay_3=torch.mul(dec3,lay3)  #element wise multiply  [1, 64, 32, 40, 40])
        
        lay2=self.sigmoid(torch.cat((self.deconv2(lay_3),dec2),dim=1)) 
        lay2 = self.decoder2(lay2) 
        lay_2=torch.mul(dec2,lay2)  # [1, 32, 64, 80, 80])
        
        lay1=self.sigmoid(torch.cat((self.deconv1(lay_2),dec1),dim=1)) 
        lay1 = self.decoder1(lay1) 
        lay_1=torch.mul(dec1,lay1)  # [1, 16, 128, 160, 160])
        
        final = self.final(lay_1)        # [1, 2, 128, 160, 160])
